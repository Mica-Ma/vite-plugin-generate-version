 ...
 * 清空指定目录中的版本文件
 * @param {string} outputPath - 输出文件目录，默认为 'public'
 * @param {Array<string>} fileTypes - 要清空的文件类型，默认为 ['json', 'js', 'txt', 'ts']
 * @returns {Object} 清空结果信息
 */
function clearVersionFiles(outputPath = 'public', fileTypes = ['json', 'js', 'txt', 'ts']) {
  try {
    // 验证输入参数
    if (typeof outputPath !== 'string' || outputPath.trim() === '') {
      throw new Error('输出路径必须是非空字符串')
    ...
    fileTypes.forEach((fileType) => {
      const fileName = `version.${fileType}`
      ...
      try {
        if (fs.existsSync(filePath)) {
          ...
          console.log(`ℹ️ 文件不存在，跳过: ${filePath}`)
        }
      } catch (error) {
    ...
    const result = {
      ...
      message:
        errors.length === 0 ? `成功清空 ${clearedFiles.length} 个版本文件` : `清空完成，但有 ${errors.length} 个错误`,
      clearedFiles,
      errors,
    }

    if (clearedFiles.length > 0) {
      console.log(`🎉 版本文件清空完成！共删除 ${clearedFiles.length} 个文件`)
    } else if (errors.length === 0) {
      console.log('ℹ️ 没有找到需要清空的版本文件')
    }

    return result
  } catch (error) {
    console.error('❌ 清空版本文件时出错:', error.message)
    return {
      success: false,
...
module.exports.clearVersionFiles = clearVersionFiles
module.exports.generateVersion = generateVersionFile
